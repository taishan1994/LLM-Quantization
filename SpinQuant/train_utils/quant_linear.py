...
class QuantizeLinear(nn.Linear):
    def forward(
        ...
        if R1 is not None:
            ...
            if not transpose:
                weight = (self.weight.to(torch.float32) @ R1.to(torch.float32)).to(
                    dtype
                )
            else:
                weight = (R1.T.to(torch.float32) @ self.weight.to(torch.float32)).to(
                    dtype
                )
            if R2 is not None:
                ...
                if transpose:
                    ...
                    temp = temp.to(torch.float32) @ R2.to(torch.float32)
