...
def cutlass_scaled_mm(a: torch.Tensor,
    ...
    scale_a.requires_grad = False
    scale_b.requires_grad = False
    if bias is not None:
        bias.requires_grad = False
    if len(a.shape) == 3:
        a = a.squeeze(1)

    if b.shape[0] % 16 != 0 or b.shape[1] % 16 != 0:
        scale_b = scale_b.transpose(1, 0)
        # print(a.shape, b.shape, scale_a.shape, scale_b.shape, bias.shape)
        if bias is None:
            out = (a * scale_a) @ (b * scale_b)
        else:
            out = (a * scale_a) @ (b * scale_b) + bias
        return out.to(torch.float16).to(a.device)
