...
def update_dataset(layer, rotary_emb, dataset, dev, attention_mask, position_ids):
    with torch.no_grad():
        with torch.cuda.amp.autocast():
            for index, inps in enumerate(dataset):
                ...
                # --- 新增：手动计算 position_embeddings ---
                position_embeddings = rotary_emb(inps, position_ids)
                new_data = layer(inps,
                                 attention_mask=attention_mask,
                                 position_ids=position_ids,
                                 position_embeddings=position_embeddings)[0].to('cpu')
...
def block_ap(
    ...
    # --- 修改：捕获后立即将 mask 和 ids 移到 GPU ---
    # 捕获 attention_mask 和 position_ids
    caught_attention_mask = layers[0].attention_mask
    position_ids = layers[0].position_ids.to(dev)

    layers[0] = layers[0].module
    # 检查捕获到的 attention_mask 是否为 None
    if caught_attention_mask is not None:
        # 如果不是 None, 才移动到 GPU 并创建 batch
        attention_mask = caught_attention_mask.to(dev)
        attention_mask_batch = attention_mask.repeat(args.batch_size, 1, 1, 1).float()
    else:
        # 如果是 None, 就保持为 None
        logger.info(
            "The caught attention_mask is None. "
            "Proceeding without an explicit attention mask."
        )
        attention_mask = None
    ...
    for block_index in range(len(layers)):
        ...
        if args.epochs > 0:
            ...
            update_dataset(qlayer, rotary_emb, fp_train_inps, dev, attention_mask, position_ids)
            update_dataset(qlayer, rotary_emb, fp_val_inps, dev, attention_mask, position_ids)
        ...
        if args.epochs>0:
            ...
            update_dataset(qlayer, rotary_emb, quant_train_inps, dev, attention_mask, position_ids)
            update_dataset(qlayer, rotary_emb, quant_val_inps, dev, attention_mask, position_ids)
