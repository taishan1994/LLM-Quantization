...
class DSFP8Method(LinearMethodBase):
    """Linear method without quantization."""
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        """
        在权重加载完成后，将 weight 转为 FP8 量化形式，并保存反量化所需的数据。
        不再保留原始大权重（可选），以节省显存。
        """
        if current_platform.is_cpu():
            from vllm.model_executor.layers.utils import dispatch_cpu_unquantized_gemm
        ...
        # 1. 对原始 weight 做 FP8 量化
        w_q, w_s = weight_cast_to_fp8(layer.weight.data, block_size=128)

        # 2. 反量化为 BF16（用于后续 GEMM）
        w_bf16 = weight_cast_to_bf16(w_q, w_s, block_size=128)

        # 3. 替换 weight 为静态 Parameter（不再改变！）
        # 注意：这里只做一次，在 apply 中不再修改
        layer.weight = nn.Parameter(w_bf16, requires_grad=False)

        # 4. 【可选】删除原始大张量（如果 weight 已被替换）
        # 实际上上面赋值已覆盖，但确保无引用
        if hasattr(layer, "_raw_weight"):
            delattr(layer, "_raw_weight")

        # 5. 如果你需要在 apply 中使用 scale（比如做 activation 量化对齐），
        #    可以注册为 buffer（但当前你的 apply 没用到 w_s，所以可省略）
        # layer.register_buffer("weight_scale", w_s, persistent=False)
    ...
    def apply(
        ...
        # w_q, w_s = weight_cast_to_fp8(layer.weight, block_size=128)
        # layer.weight = nn.Parameter(weight_cast_to_bf16(w_q, w_s, block_size=128), requires_grad=False)
        x_q, x_s = act_quant(x, block_size=128)

        x = x_q.to(torch.float32) * x_s.repeat_interleave(128, dim=-1)
        x = x.view(ori_x_shape).to(ori_x_dtype)
    ...
    def __init__(
        ...
        *,
        ...
        if quant_config is None:
            if "vpm" in prefix or "resampler" in prefix:
                self.quant_method: QuantizeMethodBase | None = UnquantizedLinearMethod()
            else:
                self.quant_method = DSFP8Method()
